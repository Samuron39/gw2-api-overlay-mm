...
- Fase 10 F (12. sept 2026): skill-bar for alle profesjoner, datadrevet fra API-et. Elementalist alle attunements (Weaver dual), Engineer toolbelt og kits/bundles, Revenant legends fra `/v2/legends`, shroud og andre transformasjoner fra `transform_skills`, Berserk, ladninger fra «Maximum Count»/«Count Recharge» med trait-avhengig Count Recharge. Aktiv variant velges i overlayen fra buffer og aktiveringer (`renderer/skillbar-logic.js`), enhetstester i `test/skills.test.js`. Rettet: profesjonsmekanikk uten våpen (weapon_type «None») ble filtrert bort, kjerne-necro fikk Scourge-F2–F5, Reaper viste Death Shroud, Firebrand «Stow Tome».
- Ikke gjort: ikoner for boons og conditions (vises som forkortelser eller navn), anbefalte builds fra nettsider (MetaBattle/Snow Crows har ingen stabil API; vurderes som eget steg), verifisering mot ekte kamp og ekte konto (broen er bare testet med syntetiske hendelser; skill-baren mot syntetiske data og offentlige API-endepunkt). Skill-bar som API-et ikke dekker: Photon Forge-skills, Specter Shadow Shroud, Thief stjålne skills, Ranger pet-skills, trait-avhengige vanlige cooldowns (API-et har bare trait-avhengig Count Recharge), Weaver dual-attunement uten buff.
