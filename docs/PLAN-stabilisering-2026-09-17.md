...
6. ~~Små ting.~~ **Ferdig 18. sept i `5b29efb` (ikke utgitt):** `electron`-vakten er lagt inn i `dps.js` og `skills.js`. Veiviseren: `setup.isComplete()` avgjør om alt påkrevd er på plass (ingen røde steg), og `setup:check` setter da `setupDone` selv, så Kom i gang ikke åpner seg ved hver start. Avhukingen virker som før.
