...
ipcMain.handle('skills:icons', async (_e, ids) => { const { byId } = await skills.fetchIndex(); const out = {}; for (const id of ids || []) if (byId[id]?.icon) out[id] = byId[id].icon; return out; }); // ikon-URL per skill-id, for effekter uten lokalt ikon
