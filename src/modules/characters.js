...
async function fetchCharacters(key) {
  ...
  const out = chars.map((c) => {
    ...
    for (const slot of PVE_SLOTS) {
      ...
      if (!e) { if (c.level >= 80 && slot !== 'WeaponB1' && slot !== 'Backpack') issues.push(`Tom slot: ${slot}`); continue; }
      ...
      if (c.level >= 80) {
        if (isArmorOrWeapon && upgrades.length === 0) issues.push(`${slot}: mangler ${it.type === 'Weapon' ? 'sigil' : 'rune'}`);
        if ((RARITY_RANK[it.rarity] ?? 0) < RARITY_RANK.Exotic && (isArmorOrWeapon || isTrinket)) issues.push(`${slot}: ${it.rarity || 'ukjent'}, under Exotic`);
        if (it.rarity === 'Ascended' && upgradeSlots > infusions.length && it.type !== 'Weapon') issues.push(`${slot}: ${upgradeSlots - infusions.length} tom(me) infusion-slot(s)`);
      }
      rows.push({ slot, id: e.id, name: it.name || `Item ${e.id}`, icon: it.icon, rarity: it.rarity, type: it.type, level: it.level, stat, upgrades, infusions, binding: e.binding || '' });
    }
    return {
...
async function review(config, charName, dpsModule, dir) {
  ...
  if (!c) throw new Error('Fant ikke karakteren.');
  ...
  const messages = [
    { role: 'system', content: 'Du er en erfaren Guild Wars 2-spiller og build-rådgiver. Svar på norsk, kort og konkret, i punktlister. Ikke finn opp stat-navn eller runer som ikke finnes.' },
