...
(() => {
  ...
  function render() {
    ...
    if (view === 'log') {
      ...
      body.innerHTML = err + g.stashSummary.map((s) => `<div class="dy-card" style="margin:8px 12px"><h3>${esc(s.note || t('guild.stashDefault'))} <span class="muted small">${gold(s.coins)} · ${esc(t('guild.slots', { used: s.items.length, size: s.size }))}</span></h3><div class="dy-list">${s.items.map((it) => `<span class="dy-item"><img src="${esc(it.icon)}" alt="" style="width:16px;height:16px;vertical-align:middle" /> ${esc(it.name)} ×${it.count}</span>`).join('') || `<span class="muted">${esc(t('guild.empty'))}</span>`}</div></div>`).join('') || `<div class="empty">${esc(t('guild.noStash'))}</div>`;
    } else {
